...
void read_env()
{
	...
	// Each line in /etc/passwd file ,contains seven attributes or fields: 
	// name, password, user ID, group ID, gecos, home directory and shell.
	//  Each attribute is separated from the adjacent attributes by colons 
	//  but with no spaces. Colons must not be used in the attributes themselves,
	//   in order to avoid confusing the system. If there is no data for an attribute,
	//    there is no space, but, rather, two consecutive colons.
	in.open("/etc/passwd");
	string line;
	...
	// environment file:/etc/environment - This file is specifically
	//  meant for system-wide environment variable settings. 
	//  It is not a script file, but rather consists of assignment expressions, 
	//  one per line. Specifically, this file stores the system-wide locale and path settings.
...
void make_rc()
{
	...
	//  the name of the user logged in on the controlling terminal
	//   of the process,
...
void execute_back(char **args ,char *buf)
{
	int pid = fork();
...
main()
{
	...
	populate_trie();
	//insert everything present in directory into TRIE
